...
async def require_po_or_director(user: dict = Depends(get_current_user)) -> dict:
    """Require the JWT to carry a PO or director role claim."""
    if user.get("role") not in ("po", "director"):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="PO or director access required")
    return user
...
async def require_congress_api_key(request: Request) -> None:
    """Validate the X-Congress-Api-Key header against the configured API key."""
    if not settings.congress_api_key:
        return
    api_key = request.headers.get("X-Congress-Api-Key", "")
    if not api_key or api_key != settings.congress_api_key:
        raise HTTPException(
...
def verify_ws_token(token: str) -> dict:
    """Validate a JWT for WebSocket connections (not async, no Depends).
